...
### Task 2: Verify strategy runner reports non-zero token savings

**Files:**
- Test: `tests/strategy-runner.test.ts`

- [ ] **Step 1: Write a test confirming non-zero tokensSaved**

Add to `tests/strategy-runner.test.ts`:

```typescript
it("reports non-zero tokensSaved when entries have tokenCount", () => {
  const state = createSessionState();
  state.currentTurn = 5;

  // Set up two duplicate tool calls with token counts
  state.toolParameters.set("c1", {
    tool: "glob",
  ...
  state.toolIdList = ["c1", "c2"];

  const result = runStrategies(state, makeDefaultConfig());

  expect(result.pruned).toBe(1);
  expect(result.tokensSaved).toBe(200);
  expect(state.stats.totalPruneTokens).toBe(200);
});
```

(Import `makeDefaultConfig` from `./helpers.ts` if not already imported.)

- [ ] **Step 2: Run the test**

Run: `pnpm vitest run tests/strategy-runner.test.ts`

Expected: PASS (this test exercises the already-existing runner logic which reads `entry.tokenCount ?? 0` — now that Phase 1 Task 1 populates it, integration will work).

- [ ] **Step 3: Commit**

```bash
git add tests/strategy-runner.test.ts
git commit -m "test: verify strategy runner reports non-zero tokensSaved"
```
